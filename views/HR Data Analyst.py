...
def load_and_preprocess_data(uploaded_file):
    ...
    # Find the columns based on the values and rename them
        ...
        for col in df.columns:
            try:
                if df[col].astype(str).str.contains(value).any() and new_name not in renamed_columns:
                    ...
                    break  # Stop after renaming one column to prevent duplicates
    ...
    # Merge dataframes based on Αριθμός μητρώου
    df = df.merge(
    ...
    # Update Σύμβαση with new data where available
    ...
    #df = df[(df['Departure Year'].isna()) | (df['Departure Year'] > 2020)]
    return df
...
if st.session_state.df is not None:
    ...
    year_input_2 = st.sidebar.number_input('Year for Hires & Departures', value=2024, min_value=2000, max_value=2100, step=1)
    selected_date = st.sidebar.date_input("Select a reference date for age group", value=pd.to_datetime('2024-12-31'))
